    ...
    ui(new Ui::SearchDialog)
{
    ...
    int w = ui->label_5->width()+30;
    int h = ui->label_5->height() + ui->buttonBox->height()+ ui->useAss->height() + ui->useGroup->height() + ui->useStud->height() + ui->useTeach->height();
    this->resize(w, h);
}
...
void SearchDialog::on_buttonBox_accepted()
{
    ...
    bool useTeach = ui->useTeach->isChecked();       // Преподаватели
    bool useStud = ui->useStud->isChecked();       // Учащиеся
    bool useAss = ui->useAss->isChecked();        // Объединения              Да-да, это любимое сокращение Dive (dive155)
    bool useGroup = ui->useGroup->isChecked();      // Группы
...
void SearchDialog::on_useStud_stateChanged(int arg1)
{
    if (arg1 == Qt::Checked)
    {
        ui->scrollArea->show();
    ...
    else
    {
        ui->scrollArea->hide();
        int w = this->width();
        int h = this->height() - ui->scrollArea->height();
        this->resize(w, h);
    }
}

void SearchDialog::on_useGroup_stateChanged(int arg1)
{
    if (arg1 == Qt::Checked)
    {
    ...
    else
    {
        ui->scrollArea_2->hide();
        int w = this->width();
        int h = this->height() - ui->scrollArea_2->height();
        this->resize(w, h);
    }
}

void SearchDialog::on_useAss_stateChanged(int arg1)
{
    if (arg1 == Qt::Checked)
    {
    ...
    else
    {
        ui->scrollArea_3->hide();
        int w = this->width();
        int h = this->height() - ui->scrollArea_3->height();
        this->resize(w, h);
    }
}

void SearchDialog::on_useTeach_stateChanged(int arg1)
{
    if (arg1 == Qt::Checked)
    {
    ...
    else
    {
        ui->scrollArea_4->hide();
        int w = this->width();
        int h = this->height() - ui->scrollArea_4->height();
        this->resize(w, h);
    }
}
