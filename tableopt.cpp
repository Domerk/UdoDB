    ...
    if (row > -1) // Если есть выделенные строки
    {
        ...
        for (QTableWidgetSelectionRange selectionRange : ui->tableWidget->selectedRanges())
        {
            if (selectionRange.rowCount() > 0)
            {
                ...
                {
                    // `Фамилия`, `Имя`, `Отчество`,`Тип документа`, `Номер документа`, `Пол`, `Год рождения`, `Район школы`, `Школа`, `Класс`, `Родители`, `Домашний адрес`, `Телефон`, `e-mail`
                    strQuery.append("INSERT INTO Учащиеся (`Фамилия`, `Имя`, `Отчество`,`Тип документа`, `Номер документа`, `Пол`, `Год рождения`, `Район школы`, `Школа`, `Класс`, `Родители`, `Домашний адрес`, `Телефон`, `e-mail`) VALUES (");
                    ...
                    qsl.append(strQuery);
                    strQuery.clear();
                    strQuery.append("INSERT INTO Нагрузка (`ID учащегося`, `ID группы`) VALUES ((SELECT `ID` FROM Учащиеся WHERE `Тип документа` = '" + ui->tableWidget->item(row, 4)->text() + "' AND `Номер документа` = '" + ui->tableWidget->item(row, 4)->text() + "'), (SELECT Группа.`ID` FROM Группа, Объединение WHERE Группа.`ID объединения` = Объединение.`ID` AND Объединение.`Название` = '" +ui->tableWidget->item(row, 0)->text()  + "' AND Группа`Номер` = 'Без группы'));");
                    qsl.append(strQuery);
                }
            }
        }
