...
void Help::on_treeWidget_itemClicked(QTreeWidgetItem *item, int column)
{
    ...
    case 13: {
        ui->textEdit->setHtml("<h2>Расширенный поиск через конструктор запросов</h2>"
                              ...
                              "В окне конструктора пользователь может отметить названия полей, которые должны быть отображены в итоговой таблице, и, при необходимости, указать значения, являющиеся критериями поиска.</p>"
                              "<p>Поиск по текстовым полям также реализован через оператор LIKE, а для поиска значений в данном интервале используются операторы сравнения.</p>");
        break;
    }
    case 14: {
        ui->textEdit->setHtml("<h2>Дополнительные функции</h2>"
                              ...
                              "<p>Для того, чтобы экспортировать текущую таблицу в файл в формате .xls или .xlsx, необходимо выбрать пункт меню <b>Файл &rarr; Экспорт в Excel</b> и в открывшемся окне выбрать существующий файл (если он не пустой, то в процессе экспорта будет перезаписан).</p>"
